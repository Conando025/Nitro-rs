...
pub fn initalize_user_settings() {
    unsafe {
        ...
        readFirmware(0x20, user_settings_baseaddr.as_mut_ptr());
        ...
        readFirmware(slot0_addr + 0x00, slot0_data.as_mut_ptr());
        readFirmware(slot0_addr + 0x70, slot0_count.as_mut_ptr());
        readFirmware(slot0_addr + 0x72, slot0_crc.as_mut_ptr());

        readFirmware(slot1_addr + 0x00, slot1_data.as_mut_ptr());
        readFirmware(slot1_addr + 0x70, slot1_count.as_mut_ptr());
        readFirmware(slot1_addr + 0x72, slot1_crc.as_mut_ptr());
        ...
        if !(slot0_valid || slot1_valid) { return }
        else if slot0_valid && slot1_valid {
            let slot0_count = slot0_count.assume_init();
            let slot1_count = slot1_count.assume_init();
            current_settings_slot = if slot1_count == (slot0_count + 1) & 0x7f { &slot1_data } else { &slot0_data };
        } else if slot1_valid {
            current_settings_slot = &slot1_data;
        }
        writeFirmware(0x2FFFC80, current_settings_slot);

    }
}

pub unsafe fn readFirmware<T>(addr: usize, ptr: *mut T) {
    todo!()
}

pub unsafe fn writeFirmware<T>(addr: usize, ptr: &T) {
    todo!()
}

pub fn swiCRC16<T>(crc: u16, data_addr: &T) -> u8 {
    todo!()
}